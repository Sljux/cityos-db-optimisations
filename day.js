...
db.getConnectionAsync()
  .then(() => loadDevices())
  .tap(devices => {
      bar = new ProgressBar(':bar :percent elapsed :elapsed eta :eta', { total : devices.length * 2 });
  })
  .map(device => {
          return loadAvg(device)
              .tap(() => bar.tick())
              .then(props =>
...
function saveAvg(device, props) {
    ...
    return db.queryAsync(`INSERT INTO sensor_day(device_id, group_id, sensor_id, value, user_id, add_on) VALUES ?`, [values])
}

function sensorAvg(deviceId, sensorId) {
    return db.queryAsync(`SELECT AVG(value) AS avg FROM sensor WHERE device_id = ${deviceId} AND sensor_id = ${sensorId} AND add_on <= DATE_SUB(NOW(), INTERVAL 30 MINUTE)`)
